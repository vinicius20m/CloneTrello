<template>
      <Layout :appName="appName">
            <Head :title="appName+ '- Lane List'" />
            <div class="row">

                  <h1 class="mt-3 col-md-9">Lanes List</h1>
            ...
            <table v-if="lanes.length" class="table mt-5">
                  ...
                  <tbody>
                        ...
                        <tr v-for="(card, index) in lanes" :key="index">
                              <td>{{card.title}}</td>
                              <td>{{card.description}}</td>
                              <td>{{card.index}}</td>
                              <td>{{card.board.title}}</td>
                              <td>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered" role="document" style="min-width: 600px">
                                          ...
                                          <div class="modal-content">
                                                ...
                                                <div style="background: #b54c4c" class="modal-header">
                                                      ...
                                                      <h4 class="modal-title" id="modalLongTitle">Deleting the Lane: <strong>{{card.title}}</strong></h4>
                                                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">

                                                      <h2 class="text-center">You really want to delete this Lane?</h2>
                                                </div>
            ...
            <h3 class="text-center mt-5" v-else>Nenhuma Lane por enquanto. Crie uma nova <Link :href="routes.create">Aqui</Link>.</h3>
      </Layout>
</template>
<script>
export default {
